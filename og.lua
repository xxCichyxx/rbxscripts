...
local function QueueScript()
    ...
    if qot then
        -- Przygotowujemy kod jako czysty tekst, aby uniknąć błędów składniowych
        local scriptPayload = [[
            repeat task.wait() until game:IsLoaded()
            task.wait(5) -- Dajemy executorowi czas na pełne "rozgrzanie się"
            
            local success = false
            local count = 0
            while not success and count < 10 do
                count = count + 1
                ...
                if s then 
                    ...
                    warn("Próba załadowania skryptu nr " .. count .. " nieudana. Ponawiam...")
                    task.wait(2)
                end
            end
        ]]
        ...
        warn("Twój executor nie wspiera queue_on_teleport!")
    end
end
