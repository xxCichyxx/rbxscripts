...
elseif gethui and gethui():FindFirstChild(guiName) then
...
MainFrame.ClipsDescendants = false -- Allow shadow/glow to show
...
local function executeScript(scriptName)
    ...
    -- Notification (Simple print for now, could be UI based)
    print("Fetching: " .. url)
    ...
    if success then
        ...
        -- Fallback for sunc if fetch fails (optional, based on previous request)
...
local function createButton(name, callback)
    ...
    Icon.Image = "rbxassetid://6031091004" -- Play icon arrow
    ...
    Button.MouseEnter:Connect(function()
        ...
        TweenService:Create(BtnText, TweenInfo.new(0.2), {Position = UDim2.new(0, 20, 0, 0)}):Play() -- Slight shift
    end)
...
-- 7. Toggle Visibility Logic with Animation
...
local function toggleGui()
    ...
    if isVisible then
        ...
        TweenService:Create(MainFrame, TweenInfo.new(0.4, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Size = UDim2.new(0, 320, 0, 450)}):Play()
        task.delay(0.4, function()
            if isVisible then MainFrame.ClipsDescendants = false end
        end)
    else
        MainFrame.ClipsDescendants = true
        local tween = TweenService:Create(MainFrame, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {Size = UDim2.new(0, 320, 0, 0)})
        tween:Play()
